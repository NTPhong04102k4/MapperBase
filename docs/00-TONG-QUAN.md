...
| 11 | [11-GITNEXUS-HUONG-DAN.md](./11-GITNEXUS-HUONG-DAN.md) | Công cụ **GitNexus** (knowledge graph cho AI agent): cài + nối MCP, toàn bộ lệnh CLI 1.6.9, 17 MCP tool, schema Cypher đo thật, 4 vòng lặp dùng hằng ngày. Mục 12 là **cạm bẫy kiểm chứng trên repo này** — quan trọng nhất: `impact` bỏ sót toàn bộ `yield call(...)` của saga, nên `impactedCount: 0` không có nghĩa an toàn |
| 12 | [12-SUPERPOWERS-HUONG-DAN.md](./12-SUPERPOWERS-HUONG-DAN.md) | Plugin **Superpowers** v6.2.0 (14 skill quy trình: brainstorming → writing-plans → TDD → verification): cách kích hoạt, 3 iron law, đường subagent, và **mục 8 = cách hoà giải với `sr` + GitNexus + `CLAUDE.md`** (luật: user instruction > skill). Mục 11: **quy ước commit đã chốt cho repo này** — commit nháp theo step TDD (agent tự làm) → gom theo module → `sr emit commit` khi confirm task; Conventional Commit không Jira; không dùng worktree (đánh đổi ở 11.10); cách chứng minh một commit tự đứng được |
